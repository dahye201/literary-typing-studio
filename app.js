...
/* Split into ~280-char pages at word boundaries */
const PAGE_SIZE = 280;
const FREE_MAX_PG = 5;
...
const BOOKS = RAW_BOOKS.map(b => {
  const pages = splitPages(b.text);
  const gs = gpc + 1;
  gpc += pages.length;
  return { ...b, pages, globalStart: gs, globalEnd: gpc, free: gs <= FREE_MAX_PG };
});
...
/* ── Typing state ──
   passage: the full text of the current page
   typed[]:  array of booleans — typed[i] = true if char i was correct
   curIdx:   number of characters typed so far (= typed.length)

   Key invariant:
     textarea.value.length === curIdx at all times
     typed[i] === (textarea.value[i] === passage[i])
*/
let passage = '';
let typed   = [];   // boolean per char
let curIdx  = 0;    // = typed.length

let curBook = 0, curPg = 0;
let tStart = null, tAcc = 0, tIdle = false, tIdleTimer = null, tTick = null;
let soundOn = true, AC = null, lastSndT = 0;
const LINE = 54;

let currentUser = null, sessions = [], detIdx = -1;
...
document.getElementById('nextPg').addEventListener('click', () => {
  ...
  const nextG = book.globalStart + curPg + 1;
  if (nextG > FREE_MAX_PG && !currentUser) { openOv('ovP'); return; }
  if (curPg + 1 < book.pages.length) loadPage(curBook, curPg + 1);
});
...
siEl.addEventListener('input', function() { libSearch = this.value.trim(); libPage = 1; acHi = -1; rl(); rAC(this.value.trim()); });
siEl.addEventListener('keydown', function(e) {
  const items = acdEl.querySelectorAll('.acr');
  if (e.key === 'ArrowDown') { e.preventDefault(); acHi = Math.min(acHi+1, items.length-1); items.forEach((el,i) => el.classList.toggle('hl', i===acHi)); }
  else if (e.key === 'ArrowUp') { e.preventDefault(); acHi = Math.max(acHi-1, 0); items.forEach((el,i) => el.classList.toggle('hl', i===acHi)); }
  else if (e.key === 'Enter' && acHi >= 0) items[acHi]?.click();
  else if (e.key === 'Escape') { acdEl.classList.remove('open'); acHi = -1; }
});
document.addEventListener('click', e => {
  if (!siEl.contains(e.target) && !acdEl.contains(e.target)) { acdEl.classList.remove('open'); acHi = -1; }
});
...
function rAC(q) {
  ...
  const matches = BOOKS.filter(b => b.title.toLowerCase().includes(ql) || b.author.toLowerCase().includes(ql)).slice(0, 5);
  ...
  matches.forEach(b => {
    ...
    d.addEventListener('click', () => { siEl.value = b.title; libSearch = b.title; libPage = 1; acdEl.classList.remove('open'); rl(); });
    acdEl.appendChild(d);
  });
...
/* ════════════════════════════════════════
   ...
   HOW IT WORKS:
   1. A transparent <textarea id="typInput"> sits over the styled display div
   2. On `input` event: read textarea.value, compare char-by-char to passage
   3. typed[i] = (textarea.value[i] === passage[i])
   4. Display re-renders from typed[] + curIdx

   Why this reliably works:
   - textarea handles ALL keyboard events natively (space, caps, special chars)
   - We never intercept keydown or compare e.key to anything
   - We just diff textarea.value against passage after every input event
   - No focus ring because CSS removes outline from textarea
════════════════════════════════════════ */
...
function focusInput() {
  typInput.focus({ preventScroll: true });
}
...
function loadPage(bookIdx, pageIdx) {
  ...
  const nextIsLast = pageIdx + 1 >= book.pages.length;
  const nextPgBtn  = document.getElementById('nextPg');
  nextPgBtn.disabled = nextIsLast;
  const nextG = book.globalStart + pageIdx + 1;
  const nextLocked = !nextIsLast && nextG > FREE_MAX_PG && !currentUser;
...
/* ══════════════════════════════════════
   THE CORE INPUT HANDLER
   Fired after every keystroke, paste attempt,
   autocorrect, etc.

   Algorithm:
   - val = current textarea content
   - For each position 0..val.length-1:
       typed[i] = (val[i] === passage[i])
   - curIdx = val.length
   - Play sound for newly added chars
══════════════════════════════════════ */
typInput.addEventListener('input', function() {
  const val     = this.value;
  const prevLen = curIdx;

  // Trim if longer than passage
  if (val.length > passage.length) {
    this.value = val.slice(0, passage.length);
    return; // re-triggers input
  }

  // ── Core: compare every char of val against passage, one-to-one ──
  // typed[i] = true  → val[i] === passage[i]  (correct)
  // typed[i] = false → val[i] !== passage[i]  (wrong, shown red)
  typed  = Array.from({ length: val.length }, (_, i) => val[i] === passage[i]);
  curIdx = val.length;

  // ── Sound: only for newly added chars (not deletion) ──
  if (val.length > prevLen) {
    const now = Date.now();
    if (now - lastSndT > 25) {
      lastSndT = now;
      const lastI     = val.length - 1;
      const isCorrect = typed[lastI];   // true/false

      if (isCorrect) { playClack();  flashKey(val[lastI]); }
      else           { playError();  flashKeyErr(val[lastI]); }

      // Carriage line-end sounds
  ...
  // Keep textarea height in sync with display div
  ...
  // ── Finish when every character has been entered ──
  // (regardless of whether some are wrong — user typed through the whole passage)
  if (curIdx >= passage.length) finish();
});

// Block paste to keep the exercise honest
typInput.addEventListener('paste', e => e.preventDefault());

/* ── RENDER TYPING DISPLAY ──
   Walk passage char by char.
   i < curIdx  → already typed: green (ck) or red (ce)
   i === curIdx → cursor position: blinking underline (cc)
   i > curIdx  → not yet typed: dim (cd)
*/
function renderTyping() {
  ...
  for (let i = 0; i < passage.length; i++) {
    ...
    if (i < curIdx) {
      h += `<span class="${typed[i] ? 'ck' : 'ce'}">${safe}</span>`;
    } else if (i === curIdx) {
...
function updateStats() {
  ...
  document.getElementById('prf').style.width = (passage.length > 0 ? curIdx / passage.length * 100 : 0).toFixed(1) + '%';
}
...
function playReturn() {
  ...
  try {
    ...
    const d = buf.getChannelData(0);
    for (let i = 0; i < d.length; i++) d[i] = (Math.random()*2-1)*0.13*Math.pow(1-i/d.length,0.9)*(1+0.5*Math.sin(i*0.1));
    const src = c.createBufferSource(); src.buffer = buf;
    const g = c.createGain();
